...
3. Chrome → `chrome://extensions` → Developer mode → **Load unpacked** → `extension/`
   After pulling a new build, click the **reload / refresh icon on the extension card**. The old content script stays loaded until you do. Runtime is `content.bundle.js` (classic IIFE, v1.0.2) — do not load `content.js` as a module.
