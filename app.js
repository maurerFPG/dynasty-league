...
(() => {
  ...
  async function refreshBaked() {
    try {
      const [bj, rj] = await Promise.all([
        fetch("data/briefs.json?v=ol6", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
        fetch("data/recs.json?v=r1106", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
      ]);
  ...
  function notifyDraftEvent(kind, extra) {
    ...
    if (key) {
      const raw = key.replace(/^Bearer\s+/i, "");
  ...
  async function init() {
    ...
    const [pj, dj, bj, rj] = await Promise.all([
      ...
      fetch("data/briefs.json?v=ol6", { cache: "no-store" })
        .then((r) => (r.ok ? r.json() : {}))
        .catch(() => ({})),
      fetch("data/recs.json?v=r1106", { cache: "no-store" })
