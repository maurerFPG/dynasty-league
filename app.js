...
(() => {
  ...
  async function refreshBaked() {
    try {
      const [bj, rj] = await Promise.all([
        fetch("data/briefs.json?v=b39", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
        fetch("data/recs.json?v=r403", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
      ]);
  ...
  async function init() {
    ...
    const [pj, dj, bj, rj] = await Promise.all([
      ...
      fetch("data/briefs.json?v=b39", { cache: "no-store" })
        .then((r) => (r.ok ? r.json() : {}))
        .catch(() => ({})),
      fetch("data/recs.json?v=r403", { cache: "no-store" })
