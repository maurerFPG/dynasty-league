...
(() => {
  ...
  async function refreshBaked() {
    try {
      const [bj, rj] = await Promise.all([
        ...
        fetch("data/recs.json?v=r1310b", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
      ]);
  ...
  async function init() {
    ...
    const [pj, dj, bj, rj] = await Promise.all([
      fetch("data/players.json?v=ecr822", { cache: "no-store" }).then((r) => r.json()),
      fetch("data/draft.json?v=mp1", { cache: "no-store" }).then((r) => r.json()),
      fetch("data/briefs.json?v=ol8", { cache: "no-store" })
        .then((r) => (r.ok ? r.json() : {}))
        .catch(() => ({})),
      fetch("data/recs.json?v=r1310b", { cache: "no-store" })
