...
const PICK_LINE =
  /(?:^|\b)(?:rd(?:\.|ound)?\s*\d+\s*[,:]?\s*)?(?:pick\s*)?(\d{1,3})\s*[.):\-–]?\s+(.+?)\s+(QB|RB|WR|TE|K|D\/ST|DST|DEF)\b(?:\s+([A-Za-z]{2,4}))?/i;

export function parsePickLine(line) {
  const text = String(line || "").replace(/\s+/g, " ").trim();
  if (!text) return null;
  let m = text.match(PICK_LINE);
  if (m) {
    const pos = normPos(m[3]);
    const name = m[2].replace(/,$/, "").trim();
    const team = canonTeam(m[4] || "") || (pos === "DEF" ? teamFromNickname(name) : "");
    return { pick_no: num(m[1]), name, pos, team };
  }
...
export function picksFromHistoryDom(root, index) {
  ...
  const nameNodes = root.querySelectorAll(
    ".pick-history .playerinfo__playername, .pick-history-table .playerinfo__playername"
  );
  nameNodes.forEach((el) => {
    ...
    const parsed = parsePickLine(text);
    const pickAttr = num(
      (row && (row.getAttribute("data-pick-number") || row.getAttribute("data-pick"))) ||
        (el.getAttribute && el.getAttribute("data-pick-number"))
    );
    const name = (el.textContent || "").trim();
    const posEl = row && row.querySelector && row.querySelector(".playerinfo__playerpos");
    const teamEl = row && row.querySelector && row.querySelector(".playerinfo__playerteam");
    const pos = normPos((posEl && posEl.textContent) || (parsed && parsed.pos) || "");
    const team = canonTeam((teamEl && teamEl.textContent) || (parsed && parsed.team) || "");
    const pick_no = (parsed && parsed.pick_no) || pickAttr;
    if (!pick_no || seen.has(pick_no)) return;
    const rec = toRecord({ pick_no, name: name || (parsed && parsed.name), pos, team }, index);
    if (!rec) return;
    seen.add(pick_no);
    out.push(rec);
  });

  if (out.length) return out.sort((a, b) => a.pick_no - b.pick_no);

  const nodes = root.querySelectorAll(
    ".pick-history-table tr, .pick-history li, .picklist--pick, [data-pick-number], [class*='pick-history'] tr"
  );
  nodes.forEach((el) => {
    const text = (el.textContent || "").replace(/\s+/g, " ").trim();
    const parsed = parsePickLine(text);
    const pick_no = (parsed && parsed.pick_no) || num(el.getAttribute("data-pick-number") || el.getAttribute("data-pick"));
    if (!parsed && !pick_no) return;
    const rec = toRecord(
      parsed || { pick_no, name: text },
      index
    );
    if (!rec || seen.has(rec.pick_no)) return;
    seen.add(rec.pick_no);
    out.push(rec);
  });
